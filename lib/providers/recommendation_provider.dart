...
final recommendationsProvider = FutureProvider<List<Anime>>((ref) async {
  ...
  final allAnime = latestReleases.map((item) {
    return Anime(
      id: item['id'] ?? '',
      title: item['name'] ?? '',
      image: item['thumbnail'] ?? item['image'],
      description: item['description'],
      genres: item['genres'] != null ? List<String>.from(item['genres']) : null,
      source: 'allanime',
    );
  }).toList();
...
final becauseYouWatchedProvider =
  ...
  final recentAnimeId = sorted.first.animeId;
  final recentAnimeTitle = sorted.first.animeTitle;
  final sourceAnime = allAnime.firstWhere(
    (anime) => anime.id == recentAnimeId,
    orElse: () => Anime(
      id: recentAnimeId,
      title: recentAnimeTitle,
      image: sorted.first.animeImage,
    ),
  );
